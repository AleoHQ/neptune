        ...
        1 => {
            ...
            for _ in 0..num_constants {
                while {
                    ...
                    if let Ok(f) = bytes_into_fr::<F>(&mut bytes) {
...
fn bytes_into_fr<F: PrimeField>(bytes: &[u8]) -> Result<F, Error> {
    assert_eq!(bytes.len(), 32);

    let mut fr_repr = <F::BigInt as Default>::default();
    fr_repr
        ...
        //     return random_int
        .read_le(&mut Cursor::new(bytes))
        .map_err(|e| Error::DecodingError)?;

    Ok(F::from_repr(fr_repr))
}
