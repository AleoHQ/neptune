...
pub fn hash_correct<'a, F, A>(p: &mut Poseidon<'a, F, A>) -> F
where
    F: PrimeField,
    A: Arity<F>,
{
...
pub fn full_round<'a, F, A>(p: &mut Poseidon<'a, F, A>)
where
    F: PrimeField,
    A: Arity<F>,
{
...
pub fn partial_round<'a, F, A>(p: &mut Poseidon<'a, F, A>)
where
    F: PrimeField,
    A: Arity<F>,
{
    ...
    quintic_s_box::<F>(&mut p.elements[0], None, None);
...
pub fn hash_optimized_dynamic<'a, F, A>(p: &mut Poseidon<'a, F, A>) -> F
where
    F: PrimeField,
    A: Arity<F>,
{
...
pub fn full_round_dynamic<'a, F, A>(
    p: &mut Poseidon<'a, F, A>,
    ...
    A: Arity<F>,
{
    ...
    if absorb_next_round_keys {
        ...
        // M^-1(S)
        let inverted_vec = matrix::apply_matrix::<F>(&p.constants.mds_matrices.m_inv, &post_vec);

        // M(M^-1(S))
        let original = matrix::apply_matrix::<F>(&p.constants.mds_matrices.m, &inverted_vec);
        ...
        p.elements
            ...
            .for_each(|(l, (pre, post))| {
                quintic_s_box::<F>(l, pre, Some(post));
            });
    } else {
        p.elements
            .iter_mut()
...
pub fn partial_round_dynamic<'a, F, A>(p: &mut Poseidon<'a, F, A>)
where
    F: PrimeField,
    A: Arity<F>,
{
...
/// offset.
fn add_round_constants<'a, F, A>(p: &mut Poseidon<'a, F, A>)
where
    F: PrimeField,
    A: Arity<F>,
{
